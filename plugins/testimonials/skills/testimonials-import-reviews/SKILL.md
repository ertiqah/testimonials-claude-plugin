...
- **For G2, Yelp, Google Maps, Google Play, Facebook, Amazon and the App Store** the preview says `"runs_on": "testimonials.ltd (included, no Apify token needed)"`: our server runs a review scraper for the user, so there is no page sample, only the most it will import (`max_reviews`) and the account's `included_import` allowance. Tell the user it fetches every review with photos and needs nothing from them, then import (no `preview`). **Never ask for an Apify token for these sites.** Each account can import up to its `monthly_allowance` of reviews a month this way; the progress results carry `included_import` (used this month, reset date).
- **It returns `"status": "use_another_route"`**: our server cannot do it. Read `why` to the user in plain words: the site refuses servers or needs a login, or the monthly allowance for included imports is used up (it names the reset date), or the plan's testimonial limit is reached. Then follow the `routes` it lists, in order: usually Route 2 (you fetch the pages yourself), then Route 3 (the user's own Apify account, optional), then Route 4 (the user pastes the page).

Server import works today for G2, Yelp, Google Maps, Google Play, Facebook, Amazon and the App Store (full history, every country), and the Chrome Web Store and Shopify App Store, plus anything the tool adds later, so always try it first.
...
3. **If your fetch stops early** (403, a captcha, a login page, or page N suddenly shows page 1 again) while you are still short of M: do not stop there. Tell the user how many you have, then offer Route 4 (they paste the remaining pages) or Route 3 (their own Apify token: `import_from_url` fetches the rest, with photos, and skips the ones you already imported).
...
### Route 3: the user's own Apify account (optional, for what is left)

Only for what Routes 1 and 2 cannot get: Trustpilot beyond what its filters show, a Route 2 site that starts refusing mid-way, or a G2 / Yelp / Google / Play / Facebook / Amazon / App Store import after the account's monthly allowance for included imports is used up. Only offer it; never require it.
...
"Server" means `import_from_url` does the whole job itself (Route 1), with no token from the user. The rest are Route 2 with the pagination shown, then Route 4 or 3 for whatever is left.
...
- **App Store**: server, full history: the included scraper walks every country's storefront. Apple's count is of star ratings, most without text, so N is usually below M: say so. If the monthly allowance is used up: App Store Connect (Ratings and Reviews) has every review if the user owns the app (ask them to paste them), or Route 3. Apple shows no reviewer photos.
- **Google Play**: server, full history, with reviewer photos. Pass the app's Play link. The count includes star-only ratings without text. If the allowance is used up and the user owns the app, Play Console > Download reports > Reviews gives CSV files with every review: ask them to attach them.
- **Google Maps / Business Profile**: server, full history, with reviewer photos and review photos. Pass the place link (google.com/maps/place/... or a maps.app.goo.gl share link). If the allowance is used up: Route 4 (open the place, click Reviews, sort by Newest, scroll to the end, click every "More", then copy). Relative dates ("2 months ago"): convert from today or leave them out.
...
- **G2**: server, full history (the included scraper; G2 refuses Claude's own fetch, often even page 1). Multi-part reviews come in with G2's own questions as labels. Reviewer photos where G2 shows them. If the allowance is used up: Route 4 page by page, or Route 3.
...
- **Yelp**: server, full history, with star ratings, reviewer photos and review photos. Yelp hides "not currently recommended" reviews from its list and its count. Only if the allowance is used up, Route 2: `?start=0`, `?start=10`, `?start=20` ... 10 per page; fetch tools often lose each review's star rating (never take it from Yelp's own "write a review" widget), so if a review's stars are not clearly attached to it, ask for the copied visible text, or import without a rating only if the user agrees.
...
- **Facebook**: server (pass the page's reviews link, e.g. facebook.com/yourpage/reviews). Facebook reviews are recommendations, not stars, so they come in without a rating. If the allowance is used up: Route 4 (the signed-in user copies the visible text from the page's Reviews tab).
- **Amazon**: server (pass the product link with its ASIN, e.g. amazon.com/dp/B0...), with reviewer photos and review photos. Amazon lists at most about 100 reviews per star level, so very large products may come in below the total. Only for the user's own product. If the allowance is used up: Route 4 (the signed-in user copies the visible text of each reviews page).
